...
const ProductForm = ({ productData, categories }: ProductFormProps) => {
  ...
  const creatingOrUpdatingProductValidation = productData
    ? !(label !== productData.name || isUploaded)
    : !isUploaded || label.length < 3
  const creatingOrUpdatingChecker = productData ? true : file !== null && isUploaded
  ...
  const updateProductHandler = async (
    ...
    const responseFromDB = await updateProduct({
      ...
      name: !newProductData.name ? null : newProductData.name,
      price: !newProductData.price ? null : newProductData.price,
      categoryId: !newProductData.categoryId ? null : newProductData.categoryId,
      imageUrl: sign !== null && sign.url ? sign.url.split('?')[0] : null,
      pathname,
    })
  ...
  const createProductHandler = async (file: File | null, newProductData: ProductSchemaType) => {
    const { categoryId, name, price } = newProductData
    setLoading(true)
    let urlForDB = ''
    if (!file) {
